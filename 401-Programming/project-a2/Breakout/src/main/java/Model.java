...
public class Model {
  ...
  public void initialiseGame() {
    ...
    int _brickCountX = width / BRICK_WIDTH;
    bricks = new GameObj[_brickCountX];

    for (int i = 0; i < _brickCountX; i++) {
      Debug.trace("Model::startGame: Making brick " + i);
      GameObj brick =
        new GameObj(
          i * BRICK_WIDTH,
          100,
  ...
  public synchronized void updateGame() {
    ...
    // **************************************************************
    for (GameObj gameObj : bricks) {
      if (ball.hitBy(gameObj) && gameObj.visible) {
        ball.changeDirectionY();
        gameObj.visible = !gameObj.visible;
      }
    }
